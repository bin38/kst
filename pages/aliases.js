    ...
    <>
      ...
      <div className="container">
        ...
        <div className="info">
          ...
          {/* 更新说明文字 */}
          <p>Use this form to create a new, secondary account (intended for Gmail only) with the address: <strong>kst_{'{your_primary_username}'}@{emailDomain}</strong>.</p>
          <p><strong>Note:</strong> This creates a separate Google Workspace account and attempts to assign a Gmail-only license via API. If license assignment fails, it may need to be done manually.</p>
        </div>
        ...
        {result && (
          <div className={`result-message ${result.type}`}>
            <p>{result.message}</p>
            {/* Display new account info only on successful creation */}
            ...
            {newAccountInfo && result.type === 'success' && (
              <div className="account-details">
                ...
                {/* Display warning if license assignment failed */}
                {newAccountInfo.warning && <p style={{color: '#856404', marginTop: '10px'}}><strong>Warning:</strong> {newAccountInfo.warning}</p>}
              </div>
            )}
          </div>
        )}
