  ...
  try {
    ...
    const createRes = await fetch(
      `https://admin.googleapis.com/admin/directory/v1/users`,
      {
        ...
        body: JSON.stringify(newUser), // 发送不含 archived:true 的数据
      }
    );
    ...
    if (!createRes.ok) {
      ...
      // 如果错误仍然是关于 'archived'，说明代码没有正确更新
      if (message.includes('archived')) {
         console.error("错误仍然包含 'archived'，请检查代码是否已移除该参数！");
      }
    ...
    // *** 步骤 2: 分配仅限 Gmail 的许可证 (使用 License Manager API) ***
    try {
      // !! 重要: 再次确认 '101031' 是正确的 SKU ID
      const productId = 'Google-Apps';
      const skuId = '101031'; // 假设这是“归档用户”或“仅Gmail”的SKU ID
      ...
      const licenseRes = await fetch(
        `https://licensing.googleapis.com/apps/licensing/v1/product/${productId}/sku/${skuId}/user`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${access_token}`, // 使用相同的 access_token
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ userId: createdUser.primaryEmail }), // 指定要分配许可证的用户邮箱
        }
      );

      if (!licenseRes.ok) {
        // 许可证分配失败不是致命错误，用户已创建，但需要手动处理许可证。
        const licenseErrorData = await licenseRes.json();
        console.warn(`用户 ${createdUser.primaryEmail} 创建成功，但分配许可证 SKU ${skuId} 失败:`, licenseErrorData);
        // 可以选择向前端返回一个警告信息
        return res.status(200).json({
          message: `辅助邮箱账号创建成功，但自动分配“仅Gmail”许可证失败。请稍后手动分配。`,
          email: createdUser.primaryEmail,
          password: generatedPassword, // 考虑移除密码
          warning: 'License assignment failed. Please assign manually.'
        });
      }

      console.log(`成功为用户 ${createdUser.primaryEmail} 分配了 SKU ${skuId} 许可证。`);

    } catch (licenseError) {
      console.warn(`尝试为用户 ${createdUser.primaryEmail} 分配许可证时出错:`, licenseError);
      // 同上，返回警告
      return res.status(200).json({
        message: `辅助邮箱账号创建成功，但在尝试分配“仅Gmail”许可证时发生错误。请稍后手动分配。`,
        email: createdUser.primaryEmail,
        password: generatedPassword, // 考虑移除密码
        warning: 'Error during license assignment. Please assign manually.'
      });
    }

    // 用户创建和许可证分配均成功
    res.status(200).json({
      message: '辅助邮箱账号（仅Gmail）创建成功。', // Secondary email account (Gmail-only) created successfully.
      email: createdUser.primaryEmail,
      password: generatedPassword, // 考虑移除密码
    });

  } catch (error) {
    console.error('创建辅助用户或分配许可证时出错:', error); // Error creating secondary user or assigning license: ...
    res.status(500).end('处理请求时发生内部服务器错误。'); // Internal server error during request processing.
  }
}
